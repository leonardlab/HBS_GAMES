...
def plot_timecourses() -> None:
    ...
    """
    fig, axs = plt.subplots(nrows=2, ncols=len(model.state_labels)/2,
    ...
    if settings["modelID"] == "synTF_chem":
        ...
        (tspace_before_ligand_addition, tspace_after_ligand_addition,
         solution_before_ligand_addition, solution_after_ligand_addition) = model.solve_single()
        tspace_after_ligand_addition = [i + max(tspace_before_ligand_addition) 
                                        for i in list(tspace_after_ligand_addition)]

        for i, label in enumerate(model.state_labels):
            axs[i].plot(tspace_before_ligand_addition, solution_before_ligand_addition[:,i],
                        linestyle = 'dotted', marker = 'None', color = 'black')
            axs[i].plot(tspace_after_ligand_addition, solution_after_ligand_addition[:,i],
                        linestyle = 'solid', marker = 'None', color = 'black')
