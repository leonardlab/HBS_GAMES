...
def plot_chi_sq_trajectory(chi_sq_list: list) -> None:
    ...
    plt.plot(range(0, len(chi_sq_list)), chi_sq_list, 
             linestyle="dotted", marker="None", color='lightseagreen')
...
def plot_parameter_distributions_after_optimization(df_opt: pd.DataFrame) -> None:
    ...
    ax1.errorbar( ExperimentalData().x,  ExperimentalData().exp_data, color = 'black', 
                 marker = 'o', yerr =  ExperimentalData().exp_error, fillstyle = 'none', 
                 linestyle = 'none',capsize = 2, label = 'Training data')
    ...
    df_opt_log = pd.melt(df_log, id_vars=['r_sq'], value_vars=settings["parameter_labels"])
