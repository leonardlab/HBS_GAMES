...
class Optimization:
    ...
    def define_best_optimization_results(df_optimization_results):
        ...
        Plots.plot_x_y(
            ...
            ExperimentalData.x_label,
            ExperimentalData.y_label,
            filename,
        ...
        Plots.plot_x_y(
            range(0, len(y)), y, "None", "None", "function evaluation", "chi_sq", filename
        )
    ...
    def define_parameters_for_opt(initial_parameters):
        ...
        for param_index in range(0, len(general_parameter_labels)):  # for each parameter
            for free_param_index in range(
                0, len(general_parameter_labels_free)
            ):  # for each free parameter

                # if param is free param, change vary to True and update bounds
                if (
                    general_parameter_labels[param_index]
                    == general_parameter_labels_free[free_param_index]
                ):
        ...
        for index_param in range(0, len(general_parameter_labels)):
            params_for_opt.add(
                general_parameter_labels[index_param],
        ...
        for null_index in range(len(general_parameter_labels), 10):
            params_for_opt.add("p" + str(null_index + 1), value=0, vary=False, min=0, max=1)
    ...
    def define_results_row(initial_parameters, results, chi_sq_list):
        ...
        items = [results.redchi, results.success, model, chi_sq_list, solutions_norm]
        item_labels = ["redchi2", "success", "model", "chi_sq_list", "Simulation results"]
    ...
    def optimize_single_initial_guess(initial_parameters, i):
        ...
        results_row, results_row_labels = Optimization.define_results_row(
            initial_parameters, results, chi_sq_list
        )
