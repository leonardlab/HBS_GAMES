...
from config import Settings, Context, ExperimentalData
...
def define_initial_guesses_for_pem_eval(
    ...
    for i, pem_evaluation_data in enumerate(pem_evaluation_data_list):
        ...
        df_new["data"] = [ExperimentalData.exp_data] * len(chi_sq_list)
...
def optimize_pem_evaluation_data(
    ...
    for i, df in enumerate(df_initial_guesses_list):
        ...
        r_sq_mean, chi_sq_mean, df_optimization_results, _ = Optimization.optimize_all(df, run_type = 'PEM evaluation')
...
def generate_pem_evaluation_data(df_global_search_results: pd.DataFrame) -> list:
    ...
    for row in df_global_search_results.itertuples(name=None):
        ...
        # Solve for raw data
        model.parameters = p
        solutions_norm_raw, chi_sq, r_sq = Solve_single.solve_single_parameter_set()
        ...
        save_pem_evaluation_data(
            solutions_norm_noise, "PEM EVALUATION DATA NOISE", count
        )
