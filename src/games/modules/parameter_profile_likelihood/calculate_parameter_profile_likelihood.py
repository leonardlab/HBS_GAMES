...
from plots.plots_parameter_profile_likelihood import plot_parameter_relationships
from plots.plots_parameter_profile_likelihood import plot_internal_states_along_ppl

def calculate_chi_sq_ppl_single_datapoint(
        fixed_val: float,
        fixed_index_in_free_parameter_list: int) -> Tuple[float, float, list]:
    '''
    ...
    problem_ppl['names'] = [x for i,x in
                            enumerate(parameter_estimation_problem_definition['names'])
                            if i != fixed_index_in_free_parameter_list]
    problem_ppl['bounds'] = [x for i,x in
                             enumerate(parameter_estimation_problem_definition['bounds'])
                             if i != fixed_index_in_free_parameter_list]
...
def calculate_ppl(
    parameter_label: str, calibrated_parameter_values: list,
    calibrated_chi_sq: float, threshold_chi_sq: float
